...
def forecast_fund_with_plot(fund_code: str, holdings_map: dict,
    ...
    for i, t in enumerate(tickers):
        ...
        fund_p5 += weights[i] * (vals[0] if not np.isnan(vals[0]) else 0)
        fund_median += weights[i] * (vals[1] if not np.isnan(vals[1]) else 0)
        fund_p95 += weights[i] * (vals[2] if not np.isnan(vals[2]) else 0)
    ...
    return {
        "fund_code": fund_code,
        "price_scenarios": {
            f"P5_{horizon}d": float(fund_p5),
            f"Median_{horizon}d": float(fund_median),
            f"P95_{horizon}d": float(fund_p95),
        },
